...
hourly %>% filter(
  ...
  scale_color_manual(name= "Year",values = c("#FC4E07", "#00AFBB"))+
  ylab("Median hourly sound pressure") +
  ggtitle("Soundscape at night when wind speed <20 km/hr")
```
...
```{r}
m <- lmerTest::lmer(spl50 ~ 0 + period * as.factor(year) + rca + 
    (1|position) + (1|ymdh), 
    data = filter(hourly, wspeed < 20 
    ))
summary(m)
...
```{r}
m <- lmerTest::lmer(spl50 ~ 0 + period * as.factor(year) + rca * wspeed + (1|position) + (1|ymdh) 
...
```{r}
m <- lmerTest::lmer(spl05 ~ 0 + period * as.factor(year) + rca + (1|position)+ (1|ymdh), 
...
Not significantly
```{r}
m <- lmerTest::lmer(spl95 ~ period * as.factor(year) + rca + (1|position) + (1|ymdh)
  , data = filter(hourly, wspeed < 20 & hr >= 9 & hr <= 17))
summary(m)
```
...
```{r}
mAR1 <- glmmTMB::glmmTMB(spl50 ~ period * as.factor(year) + 
    (1|position) +  
    (1|rca) +
...
mAR30 <- glmmTMB::glmmTMB(spl50 ~ 0 + rca * wspeed + period * year.f + 
    # poly(wspeed, 2) +
    poly(hr.c, 2) +
    (1|rca) +
    # (poly(wdir, 2)|rca) + # missing values?
...
mnlme2 <- lme(spl50 ~ period * year.f + poly(wspeed,2)*rca + poly(hr.c, 2)*rca,
    random = list(~1|position, ~1|rca), 
             data = filter(hourly2#, wspeed < 30
               ))
summary(mnlme2)
...
mGLS$data$resid <- residuals(mGLS)
...
```{r eval = F}
mGLSrca <- lme(spl50 ~ period * year.f + poly(hr.c, 2) * rca + poly(wspeed, 2) * rca
  , random = list(~1|position), 
  correlation = corAR1(form = ~ datehr|position,  value = 0.49), 
  # method="REML",
  data = filter(hourly2)
  )
mGLSrca$data$resid <- residuals(mGLSrca)
...
```{r eval = F}
mGLS20 <- lme(spl50 ~ period * year.f + #wspeed + 
    poly(hr.c, 2) * rca, 
     random = list(~1|position), 
  correlation = corAR1(form = ~ datehr|position,  value = 0.43), 
  # method="REML",
  data = filter(hourly2, wspeed < 20)
  )
mGLS20$data$resid <- residuals(mGLS20)
...
```{r}
ggplot(filter(mGLS20$data), aes(est, resid)) + geom_point(alpha = 0.2)  + geom_smooth() + 
