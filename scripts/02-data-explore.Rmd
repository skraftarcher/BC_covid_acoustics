...
```{r}
mAR120 <- glmmTMB::glmmTMB(spl50 ~ period * as.factor(year) + 
    (1|position) +  
    (1|rca) +
...
```{r}
mAR120$frame$resid <- residuals(mAR120)
mAR120$frame$est <- predict(mAR120)

ggplot(filter(mAR120$frame), aes(est, resid)) + geom_point(alpha = 0.2)  + geom_smooth() + 
  facet_wrap(~rca)
...
```{r}
mAR30 <- glmmTMB::glmmTMB(spl50 ~ period * year.f + 
    #wspeed + 
    poly(wspeed, 2) * rca +
    poly(hr.c, 2) +
    # poly(hr.c, 2) * rca +
...
```{r}
mAR30$frame$resid <- residuals(mAR30)
mAR30$frame$est <- predict(mAR30)

ggplot(filter(mAR30$frame), aes(est, resid)) + geom_point(alpha = 0.2)  + geom_smooth() + 
...
mnlme <- lme(spl50 ~ period * year.f + poly(wspeed,2) + poly(hr.c, 2),
    random = list(~1|position, ~1|rca), 
             data = filter(hourly2#, wspeed < 30
               ))
summary(mnlme)
ACF(mnlme,
...
mnlme2 <- lme(spl50 ~ period * year.f + #poly(wspeed,2)*rca + 
    poly(hr.c, 2)*rca,
    random = list(~1|position, ~1|rca), 
             data = filter(hourly2, wspeed < 20
               ))
...
```{r}
mGLS <- readRDS("models/spl50-gls-no-rca.rds")
summary(mGLS)
```
AIC 27417.22
...
mGLS20 <- nlme::lme(spl50 ~ phase * year + #wspeed + 
  ...
  #   # ,  value = 0.38
  #   ),
  correlation = corARMA(value = c(0.38, 0.24, 0.16),
    form = ~ datehr|position, p=3, q = 0),
  # method="ML",
  method="REML",
  data = filter(hourly2, wspeed < 20)
  )
...
saveRDS(mGLS20, file = "models/spl50-gls-20km-no-wind-p-3.rds")
...
```{r}
mGLS$data$resid <- residuals(mGLS, type = "normalized")
ggplot(filter(mGLS$data), aes(est, resid)) + geom_point(alpha = 0.2)  + geom_smooth() + 
  # facet_grid(rca~year.f, scales = "free")
  facet_grid(year.f~rca)
```
...
```{r}
mGLS20$data$resid <- resid(mGLS20, level = 0, type = "normalized")
...
plot(ACF(mGLS20, resType = "normalized"))
plot(ACF(mGLS20, resType = "response"))
```
...
# gam2 <-mgcv::gam(
  spl50 ~
    ...
    s(hr.drop, bs = "ps", k = 80, by = position) +
