...
### Control for potential differences in each drop and date using mixed model
```{r}
hourly$ymd.f <- as.factor(paste0(hourly$year,"-",hourly$md))
m <- lmerTest::lmer(spl50 ~ 0 + period * as.factor(year) + 
    (1|rca) + 
    (1|position) + 
    (1|ymd.f), 
    data = filter(hourly, wspeed < 20 
...
m <- lmerTest::lmer(spl50 ~ 0 + period * as.factor(year) + rca * poly(wspeed,2) + rca * poly(hr.c,2) + 
    # (1|rca) +
    (1|position) + 
    (1|ymd.f)
  , data = filter(hourly2#, wspeed < 40 #& hr >= 9 & hr <= 17
    ))
summary(m)
```
...
```{r}
m <- lmerTest::lmer(spl05 ~ 0 + period * as.factor(year) + rca +     
    # (1|rca) +
    (1|position) + 
    (1|ymd.f), 
  # data = filter(hourly, wspeed < 20 & hr >= 9 & hr <= 17))
  data = filter(hourly, wspeed < 20 & (hr < 4 | hr > 10)))
summary(m)
```
...
Not significantly
```{r}
m <- lmerTest::lmer(spl95 ~ period * as.factor(year) + #rca +     
    (1|rca) +
    (1|position) + 
    (1|ymd.f)
  , data = filter(hourly, wspeed < 20 & hr >= 9 & hr <= 17))
...
mAR120 <- glmmTMB::glmmTMB(spl50 ~ phase * year + poly(timeofday, 2) + site + 
    (1|deployment) + ar1(time-1|deployment),
...
acf(mAR120$frame, resType = "normalized")


plot(effects::Effect(c("hr.c", "rca"), mAR120, residuals=TRUE), partial.residual=list(lty="dashed"))
plot(effects::Effect(c("hr.c", "year"), mAR120, residuals=TRUE), partial.residual=list(lty="dashed"))
plot(effects::Effect(c("hr.c", "phase"), mAR120, residuals=TRUE), partial.residual=list(lty="dashed"))
```
...
mAR120$frame$resid <- residuals(mGLS20, type = "normalized")
...
Include wind speed and centred quadratic hr of the day 
(Doesnt't converge with all data and wind speed as covariate with random slopes)

```{r}
mAR1 <- glmmTMB::glmmTMB(spl50 ~ phase * year + 
    poly(wspeed, 2) * rca +
    # poly(hr.c, 2) + rca +
    poly(hr.c, 2) +
    # (1|rca) +
...
```{r}
ggplot(mAR1$frame, aes(est, resid)) + geom_point(alpha = 0.2) + geom_smooth() + facet_grid(rca~year)
plot(effects::Effect(c("wspeed", "rca"), mAR1, residuals=TRUE), partial.residual=list(lty="dashed"))
plot(effects::Effect(c("hr.c", "rca"), mAR1, residuals=TRUE), partial.residual=list(lty="dashed"))
plot(effects::Effect(c("hr.c", "year"), mAR1, residuals=TRUE), partial.residual=list(lty="dashed"))
plot(effects::Effect(c("wspeed", "year"), mAR1, residuals=TRUE), partial.residual=list(lty="dashed"))
plot(effects::Effect(c("hr.c", "phase"), mAR1, residuals=TRUE), partial.residual=list(lty="dashed"))
plot(effects::Effect(c("wspeed", "phase"), mAR1, residuals=TRUE), partial.residual=list(lty="dashed"))
```


```{r }
mnlme <- lme(spl50 ~ phase * year + poly(wspeed,2)*rca + poly(hr.c, 2)*rca,
...
```{r eval = F}
mGLS <- lme(spl50 ~ period * year.f + poly(hr.c, 2) + poly(wspeed,2)
  , random = list(~1|position), 
  # correlation = corAR1(form = ~ datehr|position,  value = 0.49), 
  correlation = corARMA(form = ~ datehr|position, p=1, q = 1),
  # method="REML",
  data = filter(hourly2)
  )
...
```{r eval = F}
mGLSrca <- lme(spl50 ~ period * year.f + 
    poly(hr.c, 2) * rca + poly(wspeed, 2) * rca
  , random = list(~1|position), 
  correlation = corAR1(form = ~ datehr|position
    ,  value = 0.49
    ),
  # correlation = corARMA(form = ~ datehr|position, p=3, q = 1),
  # method="REML",
  data = filter(hourly2)
  )
...
acf(d$resid, lag = 40)
mGLS20$data$resid2 <- residuals(mGLS20b, type = "normalized")
