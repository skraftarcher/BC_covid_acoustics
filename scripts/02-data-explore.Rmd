...
hourly %>% filter(
  ...
  # geom_violin() +
  geom_boxplot() +
  EnvStats::stat_n_text(y.pos = 118, alpha = 0.5) + 
  # geom_jitter(alpha=0.2) +
...
```{r}
m <- lmerTest::lmer(spl50 ~ 0 + period * as.factor(year) + rca + (1|position) 
...
```{r}
m <- lmerTest::lmer(spl50 ~ 0 + period * as.factor(year) + rca * wspeed + (1|position) 
  , data = filter(hourly, wspeed < 40 & hr >= 9 & hr <= 17))
summary(m)
```

### Are quietest times quieter?
Yes, but weaker effect than median
```{r}
m <- lmerTest::lmer(spl05 ~ 0 + period * as.factor(year) + rca + (1|position), 
  # data = filter(hourly, wspeed < 20 & hr >= 9 & hr <= 17))
  data = filter(hourly, wspeed < 10 & hr < 4 & hr > 0))
summary(m)
```
...
Not significantly
```{r}
m <- lmerTest::lmer(spl95 ~ period * as.factor(year) + rca + (1|position) 
  , data = filter(hourly, wspeed < 20 & hr >= 9 & hr <= 17))
summary(m)
```