...
```{r}
m <- lmerTest::lmer(spl50 ~ period * as.factor(year) + period * rca + (1|position) 
...
```{r}
m <- lmerTest::lmer(spl05 ~ period * as.factor(year) + period * rca + (1|position), 
  # data = filter(hourly, wspeed < 20 & hr >= 9 & hr <= 17))
...
Not significantly
```{r}
m <- lmerTest::lmer(spl95 ~ period * as.factor(year) + period * rca + (1|position) 
  , data = filter(hourly, wspeed < 20 & hr >= 9 & hr <= 17))
summary(m)
```