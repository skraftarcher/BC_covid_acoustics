...
```{r}
m <- lmerTest::lmer(spl50 ~ 0 + period * as.factor(year) + rca + (1|position) + (1|md) + (1|mdh)
...
```{r}
m <- lmerTest::lmer(spl50 ~ 0 + period * as.factor(year) + rca * wspeed + (1|position) + (1|mdh) #+ (1|md)
  , data = filter(hourly, wspeed < 30 & hr >= 9 & hr <= 17))
summary(m)
```
